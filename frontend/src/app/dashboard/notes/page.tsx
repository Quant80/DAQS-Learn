...
// ── Question card (shared compact/full variants) ───────────────────────────────

function QuestionCardDisplay({ msg, compact = false }: { msg: NoteMessage; compact?: boolean }) {
  ...
  return (
    <div className={`border border-sky-500/20 bg-sky-500/5 rounded-2xl p-4 space-y-3 ${compact ? "" : "mb-1"}`}>
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2 flex-wrap">
          <span className={`font-bold text-sky-400 uppercase tracking-wider ${compact ? "text-[9px]" : "text-xs"}`}>
            Question {q.number} of {q.total}
          </span>
          <span className={`text-white/30 border border-white/10 rounded px-1.5 py-0.5 capitalize ${compact ? "text-[9px]" : "text-[10px]"}`}>
            {q.qType.replace("_", " ")}
          </span>
          <span className={`text-white/30 border border-white/10 rounded px-1.5 py-0.5 ${compact ? "text-[9px]" : "text-[10px]"}`}>
            {q.difficulty}
          </span>
        </div>
        <span className={`font-bold border rounded-full px-2.5 py-0.5 shrink-0 ${scoreBadge} ${compact ? "text-[9px]" : "text-xs"}`}>
      ...
      <div className={`space-y-1.5 ${compact ? "text-[10px]" : "text-xs"}`}>
        <div className="flex items-start gap-2">
          <span className="text-white/35 w-22 shrink-0">My answer:</span>
          <span className={`font-mono leading-relaxed ${q.fullMark ? "text-emerald-300" : "text-red-300/80"}`}>{q.userAnswer}</span>
        </div>
        {!q.fullMark && (
          <div className="flex items-start gap-2">
            <span className="text-white/35 w-22 shrink-0">Correct:</span>
...
// ── Full AI-Tutor-identical renderer for the expanded panel ────────────────────

function FullNoteMessage({ msg }: { msg: NoteMessage }) {
  if (msg.type === "question_prompt" && msg.questionMeta) {
    return <QuestionCardDisplay msg={msg} compact={false} />;
  }
  if (msg.role === "user") return null; // hide raw follow-up user text
  ...
  return (
    <div className="flex gap-3">
      ...
      <div className="flex-1 rounded-2xl rounded-tl-sm px-4 py-3 text-sm leading-relaxed bg-white/[0.04] border border-white/10 text-white/90">
        <ReactMarkdown
          ...
          components={{
            code({ className, children }) {
              ...
              return (
                <code className="bg-white/10 text-emerald-300 rounded px-1.5 py-0.5 text-xs font-mono">{children}</code>
              );
            },
            ...
            table: ({ children }) => (
              <div className="my-4 overflow-x-auto rounded-xl border border-violet-500/25 shadow-lg shadow-violet-950/30">
                <style>{`
                  .daqs-note-table tbody tr:nth-child(even) { background: rgba(139,92,246,0.04); }
                  .daqs-note-table tbody tr:hover { background: rgba(96,165,250,0.06); }
                  .daqs-note-table tbody td:first-child { color: rgba(255,255,255,0.95); font-weight: 500; }
                  .daqs-note-table strong { color: #fbbf24; font-weight: 700; }
                  .daqs-note-table em { color: #a78bfa; font-style: normal; font-weight: 600; }
                `}</style>
                <table className="w-full text-sm border-collapse daqs-note-table">{children}</table>
              </div>
            ),
            thead: ({ children }) => (
            ...
            tr: ({ children }) => <tr className="border-t border-white/[0.06] transition-colors">{children}</tr>,
            th: ({ children }) => (
            ...
            td: ({ children }) => (
              <td className="px-4 py-3 text-white/75 border-r border-white/[0.04] last:border-r-0">{children}</td>
            ),
          }}
        >
...
function CompactNoteMessage({ msg }: { msg: NoteMessage }) {
  if (msg.type === "question_prompt" && msg.questionMeta) {
    return <QuestionCardDisplay msg={msg} compact />;
  }
  ...
  return (
    <div className="flex gap-3">
      ...
      <div className="flex-1 rounded-2xl rounded-tl-sm px-3.5 py-2.5 text-xs leading-relaxed bg-white/[0.03] border border-white/8 text-white/80">
        <ReactMarkdown
          ...
          components={{
              ...
              if (lang) {
                return (
                  <div className="my-2 rounded-xl overflow-hidden border border-white/10">
                    <div className="bg-white/5 px-3 py-1 border-b border-white/8">
            ...
            th: ({ children }) => <th className="px-3 py-2 text-left text-[9px] font-bold text-white uppercase">{children}</th>,
...
function ExpandedNotePanel({ note, onClose }: { note: TutorNote; onClose: () => void }) {
  ...
  return (
    // Covers the full right panel — md:left-56 matches sidebar w-56 = 224px
    <div className="fixed inset-0 md:left-56 z-40 flex flex-col bg-[#060d1a]">
      {/* Header */}
      <div className="flex items-center gap-4 px-5 py-3.5 border-b border-white/8 shrink-0 bg-[#070f20]">
        <button
          onClick={onClose}
        ...
        <span className="text-white/20 text-[10px] hidden md:block shrink-0">
      ...
      {/* Full-width scrollable content — matches AI Tutor messages area exactly */}
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 space-y-5">
        {note.messages.map((msg, i) => (
          <FullNoteMessage key={i} msg={msg} />
        ))}
        <div className="h-8" />
      </div>
    </div>
