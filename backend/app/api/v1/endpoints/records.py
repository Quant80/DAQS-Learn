...
from app.services.course_access import get_unlocked_course_ids, can_access_course
...
async def enroll(course_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    if not can_access_course(course_id, current_user, unlocked):
        raise HTTPException(status_code=403, detail="This course requires a Pro plan, an active promo spot, or an admin-granted unlock.")
